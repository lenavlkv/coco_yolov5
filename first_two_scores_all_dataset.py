...
def results_predict(img_path, model, hooks, threshold=0.5, iou=0.7, save_image = False, category_mapping = None):
    ...
    shape = detect_hook.input[0][0].shape
    x = []
...
def main():
    ...
    for img_id in imgIds:
        ...
        print("Processed", len(results), "boxes for image:", img_path)

        for j in results:
            sorted_activations = sorted(j['activations'], reverse=True)[:2]

            print(sorted_activations)
